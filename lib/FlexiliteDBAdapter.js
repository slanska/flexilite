...
///<reference path="../d.ts/sqlite3/sqlite3.d.ts"/>
...
(function (Flexilite) {
    ...
    var FlexiliteDB = (function (_super) {
        ...
        function FlexiliteDB(schema, client /*sqlite3.SQLite3*/) {
            ...
            this.alterTable = function (model, actualFields, indexes, done, checkOnly) {
                ...
                for (var fieldName in m.properties) {
                    ...
                    if ('undefined' !== typeof idx['index'] || 'undefined' !== typeof idx['unique']) {
                ...
                function changed(newSettings, oldSettings) {
                    ...
                    if (oldSettings.notnull !== notnull || oldSettings.dflt_value !== dflt_value) {
                        return true;
                    }
                ...
                function rebuid(model, oldSettings, newSettings, indexes, done) {
                    ...
                    var inSql = 'INSERT INTO `' + model + '` (' + nsst.join(',') + ') ' + 'SELECT ' + nsst.join(',') + ' FROM `tmp_' + model + '`;';
        ...
        FlexiliteDB.prototype.save = function (model, data, callback) {
            ...
            var sql = 'UPDATE ' + this.tableEscaped(model.modelName) + ' SET ' + Object.keys(data).map(function (key) {
            ...
            if (val.constructor.name === 'Object') {
                ...
                if (operator === 'between') {
                    if (prop.type.name === 'Date') {
                        return 'strftime(' + this.toDatabase(prop, val[0]) + ')' + ' AND strftime(' + this.toDatabase(prop, val[1]) + ')';
                    }
                    else {
        ...
        FlexiliteDB.prototype.createTable = function (model, indexes, done) {
            ...
            for (var prop in model.properties) {
                ...
                var ctlo = 0; // TODO
                var ctloMask = 0; // TODO
                sql += util.format(FlexiliteDB.buildCreateClassSQL(propClassName) + "insert or replace into [ClassProperties] ([ClassID], [PropertyID], [TrackChanges], " + "[ctlo], [ctloMask], [DefaultDataType], [MinOccurences], [MaxOccurences]," + "[Unique], [MaxLength]) values ((select ClassID from Classes where ClassName = '%s')," + "(select ClassID from Classes where ClassName='%s'), %d, %d, %d, '%s', %d, %d, %d, %d)", className, propClassName, prop.trackChanges, ctlo, ctloMask, prop.defaultDataType, prop.minOccurence, prop.maxOccurence);
            }
            self.client.exec();
        };
        ...
        FlexiliteDB.prototype.propertySettingsSQL = function (model, prop) {
            ...
            return this.datatype(p) + ' ' + (p.allowNull === false || p['null'] === false ? 'NOT NULL' : 'NULL');
        };
