    ...
    var SQLiteDataRefactor = (function () {
        ...
        SQLiteDataRefactor.prototype.generateClassAndSchemaDefForSync = function (model) {
            ...
            // Class already exists. It would be ALTER CLASS rather than CREATE CLASS
            {
                ...
                var newProps = {};
                _.forEach(converter.targetClass, function (p, propName) {
                    var np = {};
                ...
                var replaceSchema = '';
                var newBoxedObjProps = _.forEach(converter.targetClass, function (p, propName) {
                    if ((p.rules.type === 5 /* OBJECT */ || p.rules.type === 12 /* LINK */)
                        && !(existingProps[propName].Data.rules.type === 5 /* OBJECT */
                            || existingProps[propName].Data.rules.type === 12 /* LINK */)) {
                        if (replaceSchema === '')
                            replaceSchema = "json_set([Data] ";
                        var propID = existingProps[propName].PropertyID;
                        replaceSchema += ", json_set(Data, '$.properties." + propID + "', 'jsonPath: " + '' + "')";
                    }
                });
                if (replaceSchema !== '') {
                    replaceSchema += ")";
                    self.DB.run.sync(self.DB, "update [.schemas] set [Data] = " + replaceSchema + " where ClassID = $ClassID", { $ClassID: classDef.ClassID });
                }
