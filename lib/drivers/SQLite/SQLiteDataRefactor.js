...
var SQLiteDataRefactor = (function () {
    ...
    SQLiteDataRefactor.prototype.applyClassDefinition = function (classDef, schemaDef) {
        ...
        _.forEach(classDef.Properties, function (p, propID) {
            ...
            var colLetter = colMap[p.PropertyID];
            if (colLetter) 
        ...
        viewSQL += self.generateTriggerBegin(viewName, 'insert', 'whenNull', 'when new.[ObjectID] is null ');
        // Generate new ID
        ...
        for (var propID in classDef.Data.properties) {
            ...
            if (colLetter) {
                ...
                var jsp = schemaDef.Data.properties[propID].map.jsonPath;
    ...
    SQLiteDataRefactor.prototype.getSchemaByID = function (schemaID) {
    ...
    SQLiteDataRefactor.prototype.generateClassAndSchemaDefForSync = function (model) {
        ...
        this.assignColumns(self, vars, 2 /* REQUIRED */);
        this.assignColumns(self, vars, 1 /* DESIRED */);
        // Set class properties
        // ctloMask
        vars.classDef.ctloMask = 0 /* NONE */;
        // Column assignments
        for (var idx = 0; idx < SQLiteDataRefactor.COLUMN_LETTERS.length; idx++) {
            var ch = SQLiteDataRefactor.COLUMN_LETTERS[idx];
            ...
            if (propID) {
                ...
                if (p.Data.unique || (p.Data.role & 8 /* Code */) || (p.Data.role & 4 /* ID */)) {
    ...
    SQLiteDataRefactor.prototype.determineColAssignmentPriority = function (cp) {
        var prior = 0 /* NOT_SET */;
        if ((cp.role & 4 /* ID */) || (cp.role & 8 /* Code */) || cp.unique || cp.indexed)
            prior = 2 /* REQUIRED */;
        else {
